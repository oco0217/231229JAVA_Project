...
function tagINFO(t,u,o,p,obj)
{
	...
	for (var k in obj){
		if( obj.hasOwnProperty(k) ){
			...
			if(t=="Item") {
				_adn_items=obj[k]["i"];
				across_adn_prd_name = obj[k]["s"];
			}
		}
		items_data += "@@";}

		if( across_adn_prd_name=="" || across_adn_prd_name==null ||  typeof  across_adn_prd_name==undefined){
		}else{
			if(!_adn_prd_log_status)
			{

				var prd_url = "https://log.acrosspf.com/kd?ad_ids="+across_rb_adn_uid+"&k="+encodeURIComponent(across_adn_prd_name)+"&t="+Math.floor(Math.random()*99999999999);
				...
				try{
					...
					try{
						...
						try {
							var across_adn_prd=document.createElement("script");
							across_adn_prd.type="text/javascript";
							across_adn_prd.charset='UTF-8';
							across_adn_prd.defer=true;
							across_adn_prd.src=prd_url;
							document.getElementsByTagName("head")[0].appendChild(across_adn_prd);
						}catch(e){}
					}
				}

				_adn_prd_log_status = true;
			}
